...
// 首页
struct IndexPageController: RouteCollection, @unchecked Sendable {
    ...
    func boot(routes: any RoutesBuilder) throws {
        let posts = routes.grouped("index")
            .grouped(UserAuthenticatorMiddleware())
        // GET 请求：页面展示
        posts.get(use: indexPage) // 首页
    }
    ...
    func indexPage(req: Request) async throws -> View {
        ...
        return try await req.view.render("index/index", context)
    }
}
